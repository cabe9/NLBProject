...
The current measurable achievement is a validation-selected STNDT-lite Screen C winner that improves past the prior identity+spike public-test level (`0.3413 co-bps`) and reaches **`0.3649 co-bps`** locally. It remains below the frozen STNDT ensemble target (`0.3862 co-bps`), so the next useful step is further validation-gated model changes, not blind ensemble scaling. See [`docs/results.md`](docs/results.md).
