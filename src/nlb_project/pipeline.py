...
def _write_full_val_candidate_leaderboard(
    rows: list[tuple[float, str]],
    out_dir: Path,
) -> Path:
    """Write ranked full train/val candidate scores (highest co-bps first)."""
    ranked = sorted(rows, key=lambda item: item[0], reverse=True)
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    path = out_dir / f"full_val_candidate_leaderboard_{stamp}.txt"
    lines = ["rank\tco_bps\tlabel"]
    for rank, (score, label) in enumerate(ranked, start=1):
        lines.append(f"{rank}\t{score:.6f}\t{label}")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    logger.info("Wrote full-val candidate leaderboard -> %s", path)
    return path


def _select_best_params_full_val(
    dataset: NWBDataset,
    cfg: ExperimentConfig,
    spec: ModelSpec,
) -> dict[str, Any]:
    """Pick the best explicit candidate by full train/val co-bps (no CV folds)."""
    best_score = -np.inf
    best_params: dict[str, Any] | None = None
    leaderboard_rows: list[tuple[float, str]] = []
    for params, label in iter_cv_candidates(spec, cfg):
        ...
        score = float(metrics["co-bps"])
        leaderboard_rows.append((score, label))
        logger.info("Full-val candidate %s -> co-bps %.4f", label, score)
        if score > best_score:
            best_score = score
            best_params = params

    assert best_params is not None, f"No candidates for model `{spec.name}`"
    _write_full_val_candidate_leaderboard(leaderboard_rows, ensure_dir(cfg.output_dir))
    logger.info(
        "Selected params for %s: %s (full train/val co-bps %.4f)",
        spec.name,
        best_params,
        best_score,
    )
...
def _select_best_params(
    ...
    """Select improvement params via CV or full train/val (config-controlled)."""
    if cfg.improvement.get("select_on_full_val_all_candidates"):
        return _select_best_params_full_val(dataset, cfg, spec)
