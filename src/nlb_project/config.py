...
_OPTIONAL_IMPROVEMENT_KEYS = frozenset(
    {
        "cv_folds",
        "output_head",
        "log_offset",
        "candidates",
        "select_on_full_val_all_candidates",
    }
)
...
def _validate_improvement(section: Any, spec: ModelSpec) -> dict[str, Any]:
    ...
    if "select_on_full_val_all_candidates" in section:
        flag = section["select_on_full_val_all_candidates"]
        if not isinstance(flag, bool):
            raise ValueError(
                f"improvement[{spec.name}].select_on_full_val_all_candidates: "
                f"expected bool, got {flag!r}"
            )
        if flag and not has_explicit_candidates:
            raise ValueError(
                f"improvement[{spec.name}]: select_on_full_val_all_candidates requires `candidates`"
            )
